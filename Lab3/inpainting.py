...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/best_val.pth', help='load ckpt')
    ...
    #MVTM parameter
    parser.add_argument('--sweet-spot', type=int, default=16, help='sweet spot: the best step in total iteration')
    ...
    print("image length: ", len(t.mi_ori.dataset))
    i=0
    from tqdm import tqdm
    for i, (image, mask) in enumerate(tqdm(zip(t.mi_ori, t.mask_ori), total=len(t.mi_ori), desc="Inpainting Progress")):
